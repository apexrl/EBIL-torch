...
class DEEN(TorchBaseAlgorithm):
    def __init__(
        ...
        **kwargs
        ...
        assert mode in ['deen'], 'Invalid mode!'
    ...
    def _do_update_step(self, epoch, use_expert_buffer=True):
        ...
        if self.mode == 'deen':
            self.noise = torch.normal(torch.zeros(self.input_dim), self.sigma*torch.ones(self.input_dim))
            batch_data = torch.cat([obs, acts], dim=1)
    ...
    def evaluate(self, epoch):
        ...
        random_input = torch.cat([obs, acts], dim=1)
        ...
        statistics['expert_avg_energy'] = self.ebm(exp_input).mean().item()
